...
class GetModulesFromStreamTest extends DependencyHandlerTest
{
    ...
    public function testGettingModulesFromStream(): void
    {
        ...
        $this->assertEquals([],
            $this->uut->invoke($this->methodHandler, $this->downstreamModule, $dependencies, false));
        ...
        $this->assertSame(
            ...
            $this->alphabeticalByValues(
                $this->uut->invoke($this->methodHandler, $this->downstreamModule, $dependencies, true)
            )
        );
        $this->assertSame(
            $this->alphabeticalByValues([$this->upstreamModule, $this->moduleInBetween]),
            $this->alphabeticalByValues(
                $this->uut->invoke($this->methodHandler, $this->blueCollarModule, $dependencies, true)
            )
        );
        ...
        $this->assertSame(
            ...
            $this->alphabeticalByValues(
                $this->uut->invoke($this->methodHandler, $this->upstreamModule, $dependencies, false)
            )
        );
    }
}
