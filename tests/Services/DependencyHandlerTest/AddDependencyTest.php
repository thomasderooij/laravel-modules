...
class AddDependencyTest extends DependencyHandlerTest
{
    ...
    public function testAddingCircularReference () : void
    {
        ...
        // The dependency should be new
        $this->methodHandler->shouldReceive("dependencyExists")->withArgs([$this->upstreamModule, $this->downstreamModule])->andReturn(false);

        // But if it created a circle argument, it should throw an exception
        $this->methodHandler->shouldReceive("wouldCreateCircularReference")->withArgs([$this->upstreamModule, $this->downstreamModule])->andReturn(true);

        // There should be an exception
        $this->expectException(CircularReferenceException::class);
        $this->expectExceptionMessage("module \"{$this->downstreamModule}\" is already upstream of \"{$this->upstreamModule}\".");
