...
class WouldCreateCircularReferenceTest extends DependencyHandlerTest
{
    ...
    public function testWouldCreatedCircularReference(): void
    {
        ...
        $this->methodHandler->shouldReceive("getDownstreamModules")->withArgs([$this->moduleInBetween, $dependencies]
        )->andReturn([$this->upstreamModule]);
        $this->methodHandler->shouldReceive("getDownstreamModules")->withArgs([$this->downstreamModule, $dependencies]
        )->andReturn([]);
