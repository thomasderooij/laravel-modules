...
class ObserverMakeCommandTest extends MakeTest
{
    public function testWithoutModule () : void
    {
        // If I want to make a observer for my module
        $response = $this->artisan("make:observer", ["name" => $observer = "MyNewObserver"]);
        ...
        // We should then check if this observer already exists
        $fileDirectory = "Observers";
        $fileName = "$observer.php";
        $this->setFileExpectations($fileDirectory, $fileName, false);

        // The observer stub should be fetched
        $this->fetchStub();

        // The observer should then be created
        $capture = null;
    ...
    public function testWithModule () : void
    {
        ...
        $response = $this->artisan("make:observer", ["name" => $observer = "MyNewObserver", "--module" => $module = "MyModule"]);
    ...
    {
        $stub = realpath(__DIR__ . "/../../../vendor/laravel/framework/src/Illuminate/Foundation/Console/stubs/observer.plain.stub");
