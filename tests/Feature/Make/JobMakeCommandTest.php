...
class JobMakeCommandTest extends MakeTest
{
    public function testWithoutModule () : void
    {
        // If I want to make a job for my module
        $response = $this->artisan("make:job", ["name" => $job = "MyNewJob"]);
        ...
        // We should then check if this job already exists
        $fileDirectory = "Jobs";
        $fileName = "$job.php";
        $this->setFileExpectations($fileDirectory, $fileName, false);

        // The job stub should be fetched
        $this->fetchStub();

        // The job should then be created
        $capture = null;
    ...
    public function testWithModule () : void
    {
        ...
        $response = $this->artisan("make:job", ["name" => $job = "MyNewJob", "--module" => $module = "MyModule"]);
    ...
    {
        $stub = realpath(__DIR__ . "/../../../vendor/laravel/framework/src/Illuminate/Foundation/Console/stubs/job.queued.stub");
