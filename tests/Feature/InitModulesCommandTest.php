...
class InitModulesCommandTest extends CommandTest
{
    ...
    public function testInitModules(): void
    {
        ...
        $this->filesystem->shouldReceive("move")->withArgs(
            [base_path("bootstrap/app.php"), base_path("bootstrap/app_orig.php")]
        )->once();
        $bootstrapStubLocation = realpath(__DIR__ . "/../../src/Factories/stubs/bootstrapFile.stub");
        $this->filesystem->shouldReceive("get")->withArgs([$bootstrapStubLocation])->andReturn(
            $filesystem->get($bootstrapStubLocation)
        )->once();
        $newBootstrapFileContent = null;
        $this->filesystem->shouldReceive("put")->withArgs(
            [base_path("bootstrap/app.php"), Mockery::capture($newBootstrapFileContent)]
        )->once();

        // Creating the config file
        $configStubLocation = realpath(__DIR__ . "/../../src/Factories/stubs/config.stub");
        $this->filesystem->shouldReceive("get")->withArgs([$configStubLocation])->andReturn(
            $filesystem->get($configStubLocation)
        )->once();
        $configFileContent = null;
        $this->filesystem->shouldReceive("put")->withArgs(
            [config_path("modules.php"), Mockery::capture($configFileContent)]
        )->once();
        $appFileContent = null;
        $this->filesystem->shouldReceive("get")->withArgs([config_path("app.php")])->andReturn(
            $filesystem->get(config_path("app.php"))
        )->once();
        $this->filesystem->shouldReceive("put")->withArgs([config_path("app.php"), Mockery::capture($appFileContent)]
        )->once();
        ...
        $this->filesystem->shouldReceive("get")->withArgs([$trackerFileLocation])->andReturn(
            $filesystem->get($trackerFileLocation)
        )->once();
        $trackerFileContent = null;
        $this->filesystem->shouldReceive("put")->withArgs(
            [base_path("$this->root/.tracker"), Mockery::capture($trackerFileContent)]
        )->once();

        // Add namespace to autoload
        $composerFileLocation = base_path("composer.json");
        $this->filesystem->shouldReceive("get")->withArgs([$composerFileLocation])->andReturn(
            $filesystem->get($composerFileLocation)
        )->once();
        $composerFileContent = null;
        $this->filesystem->shouldReceive("put")->withArgs(
            [$composerFileLocation, Mockery::capture($composerFileContent)]
        );

        // Creating the migration
        $migrationStubFileLocation = realpath(__DIR__ . "/../../src/Factories/stubs/moduleMigration.stub");
        $this->filesystem->shouldReceive("get")->withArgs([$migrationStubFileLocation])->andReturn(
            $filesystem->get($migrationStubFileLocation)
        )->once();
        $migrationFileContent = null;
        $this->filesystem->shouldReceive("put")->withArgs(
            [
                database_path("migrations/2010_11_01_000000_module_init_migration.php"),
                Mockery::capture($migrationFileContent)
            ]
        )->once();
    ...
    public function testModulesAreAlreadyInitialised(): void
    {
        ...
        $this->filesystem->shouldReceive("get")->withArgs([base_path("Modules/.tracker")])->andReturn(
